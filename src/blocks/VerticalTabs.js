...
export default function VerticalTabs({ data }) {
    ...
    return (
        <section inview={inView} ref={ref} className={clsx('block block__vertical-tabs', {'in-view': inView})}>
            <div className="container">
                <div className="vertical-tabs__intro">
                    <div>
                        {intro_heading && (
            ...
            <div className="horizontal-container">
                <div className="vertical-tabs__selectors">
                    {tabs?.length > 0 &&
                        tabs?.map((item, i) => {
                            const tab = item?.tab;
                            ...
                            return (
                                <button data-tab={tab_selector_formatted} className={clsx('vertical-tabs__selectors-button', isFirst)} key={i}>{tab_selector}</button>
                            );
                        })
                    }
                </div>
                ...
                <div className="vertical-tabs__tabs">
                    {tabs?.length > 0 &&
                        tabs?.map((item, i) => {
                            ...
                            return (
                                <div className={clsx('vertical-tabs__tabs-tab', `vertical-tabs__tabs-tab--${tab_selector_formatted}`, isFirst) } key={i}>
